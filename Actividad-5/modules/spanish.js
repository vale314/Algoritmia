...
module.exports= class spanishDb
{
	...
	async analyseTextAsync(texto)
	{
		...
		//Itero palabra por palabra
		bloques.forEach(async(elem)=>
		{			
			//Traigo los grupos que pertenece cada palabra.
			let token = this.findWordTokenAsync(elem);
	...
	//Parseo la oración y la obtengo en forma de array.
	async analyseTextArrayAsync(texto)
	{	
		let resu   = await this.analyseTextAsync(texto);
		let salida = [];

		resu.forEach(async (item)=>{
			salida[item.word]=item.token;
		})
